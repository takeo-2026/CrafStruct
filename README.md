![version](https://img.shields.io/badge/version-01-blue)
![platform](https://img.shields.io/badge/platform-Windows-blue)
# 業務フロー & 業務要件定義書 作成ツール — CrafStruct

**CrafStruct** は、**業務フロー**と**業務要件定義書**を統合的に作成できる Windows デスクトップツール です。業務フローで作成した内容をもとに業務要件定義書が自動生成され、両者の編集内容は相互に反映されます。インストール不要のポータブルアプリとして ZIP 形式で提供しており、解凍して exe を実行するだけで利用できます。
...
- **レジストリ不使用**  
ZIP 形式で配布しており、インストール不要・レジストリ未使用で安心して利用できます。
...
📥 ダウンロード
---
Releases ページから ZIP をダウンロードしてご利用ください。  
👉 https://github.com/takeo-2026/CrafStruct/releases/latest (github.com in Bing)
...
前回アクセスしたプロジェクトフォルダの場所を記憶します。  
初回起動時にはこのファイルは存在せず、アプリ起動後に自動生成されます。
...
📦 セットアップ（導入手順）
---
ダウンロードした ZIP ファイルを 展開（解凍） してください。  
展開してできたフォルダを、デスクトップなど任意の場所に配置します。  
フォルダ内の CrafStruct.exe を実行するとアプリが起動します。  
※インストール作業やレジストリ登録は不要です。
